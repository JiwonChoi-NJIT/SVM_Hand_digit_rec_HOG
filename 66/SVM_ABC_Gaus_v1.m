function weight = SVM_ABC_v1(train_input,train_output)


%% Calculate Weights
N = length(train_output);                           % Length of inputs determined by looking at mapping 
X = [ones(N,1) 1/sqrt(2*pi)*exp((train_input.^2)/2)];
...
'start finding'
for j = 1:total
    w = [w_temp + 1/2000*randn(length(w_temp),1)];
    a = x_train*w;
    check = t.*a;
    P_c = sum(check>0)/N;
    weight_hold(:,j) = w;
...
for iteration = 1:1e4
    ...
    fitness_temp = [];
    for e = 1:employ
        w = weight_hold(:,e) + 1/1000*randn(length(w_temp),1);
        a = x_train*w;
        check = t.*a;
        P_c = sum(check>0)/N;
        weight_temp(:,e) = w;
    ...
    for o = 1:onlook
       index = sum(pr<rand)+1;
       w = weight_hold(:,o) + 1/1000*randn(length(w_temp),1);
       a = x_train*w;
       check = t.*a;
       P_c = sum(check>0)/N;
       weight_temp(:,o) = w;
    ...
    for s = 1:scout
        w = [w_temp + 1/200*randn(length(w_temp),1)];
        a = x_train*w;
        check = t.*a;
        P_c = sum(check>0)/N;
       weight_temp(:,s) = w;
...
for iteration = 1:1e6
    ...
    fitness_temp = [];
    for e = 1:employ
        w = weight_hold(:,e) + 1/500*randn(length(w_temp),1);
        a = x_train*w;
        check = t.*a;
        P_c = sum(check>0)/N;
        if P_c ~= 1
    ...
    for o = 1:onlook
       index = sum(pr>rand)+1;
       w = weight_hold(:,o) + 1/500*randn(length(w_temp),1);
       a = x_train*w;
       check = t.*a;
       P_c = sum(check>0)/N;
       if P_c ~= 1
